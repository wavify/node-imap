...
var CH_LF = 10,
    ...
    RE_LISTCONTENT = /^\((.*)\)$/,
    RE_LITERAL = /\{(\d+)\}$/,
    RE_UNTAGGED = /^\* (?:(OK|NO|BAD|BYE|FLAGS|ID|LIST|XLIST|LSUB|SEARCH|STATUS|CAPABILITY|NAMESPACE|PREAUTH|SORT|THREAD|ESEARCH|QUOTA|QUOTAROOT|ANNOTATION)|(\d+) (EXPUNGE|FETCH|RECENT|EXISTS))(?:(?: \[([^\]]+)\])?(?: (.+))?)?$/i,
...
Parser.prototype._resUntagged = function() {
  ...
  if (m = RE_BODYLITERAL.exec(this._buffer)) {
    ...
    if (type === 'flags'
      ...
      val = parseAnnotation(m[5], this._literals);
    else
      val = m[5];
...
function parseAnnotation(text, literals) {
  var r = parseExpr(text, literals),
      entries = {};

  for (var i = 1, len = r.length; i < len; i += 2) {
    var attValues = r[i + 1],
        attributes = {};
    for (var j = 0, len2 = attValues.length; j < len2; j += 2) {
      attributes[attValues[j]] = attValues[j + 1];
    }
    entries[r[i].toLowerCase()] = attributes;
  }
  return {
    mailbox: r[0],
    entries: entries

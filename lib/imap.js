...
var CRLF = '\r\n',
    ...
    RE_CMD = /^([^ ]+)(?: |$)/,
    RE_ISHEADER = /HEADER/,
    REX_UNRESPDATA = XRegExp('^\\* (?:(?:(?<type>NAMESPACE) (?<personal>(?:NIL|\\((?:\\(.+\\))+\\))) (?<other>(?:NIL|\\((?:\\(.+\\))+\\))) (?<shared>(?:NIL|\\((?:\\(.+\\))+\\))))|(?:(?<type>FLAGS) \\((?<flags>.*)\\))|(?:(?<type>LIST|LSUB|XLIST) \\((?<flags>.*)\\) (?<delimiter>"[^"]+"|NIL) (?<mailbox>.+))|(?:(?<type>(SEARCH|SORT))(?: (?<results>.*))?)|(?:(?<type>STATUS) (?<mailbox>.+) \\((?<attributes>.*)\\))|(?:(?<type>QUOTAROOT) (?<mailbox>"[^"]+"|\\S+)(?<quotaroots>.*))|(?:(?<type>QUOTA) (?<quotaroot>.+) \\((?<resources>.*\\)))|(?:(?<type>ANNOTATION) (?<mailbox>"[^"]+"|\\S+) (?<entries>.*))|(?:(?<type>CAPABILITY) (?<capabilities>.+))|(?:(?<type>BYE) (?:\\[(?<code>.+)\\] )?(?<message>.+)))[ \t]*(?:\r\n|$)', 'i'),
    ...
    if (indata.line[0] === '*') { // Untagged server response
      ...
      if (m = XRegExp.exec(indata.line, REX_UNRESPNUM)) {
        ...
        switch (m.type) {
          ...
          case 'ANNOTATION':
            // m.mailbox = mailbox name (string)
            // m.entries = entry attribute-value pairs list
            // TODO: unsolcited annotation response
            m.mailbox = utf7.decode(''+parsers.convStr(m.mailbox, indata.literals));
            var ret, entry;
            if (requests[0].cbargs.length)
              ret = requests[0].cbargs[0];
            else
              requests[0].cbargs.push(ret = {});
            if (!ret[m.mailbox])
              ret[m.mailbox] = {};
            if (m.entries) {
              m.entries = parsers.parseExpr(m.entries, indata.literals);
              for (i=0,len=m.entries.length; i<len; ++i) {
                entry = ret[m.mailbox][m.entries[i]];
                if (!entry)
                  entry = ret[m.mailbox][m.entries[i]] = {};
                for (var j=0,lenj=m.entries[++i].length; j<lenj; ++j)
                  entry[m.entries[i][j]] = m.entries[i][++j];
              }
            }
            break;
          case 'CAPABILITY':
      ...
      if (typeof requests[0].callback === 'function') {
        ...
        // Empty result for getAnnotation callback
        if (cmdstr.indexOf('GETANNOTATION') === 0 && args.length === 0)
          args.unshift({});
        if (m) {
          var msg = m[3], info;
          if (m[2]) {
      ...
      requests.shift();
      if (requests.length > state.pending
        && (requests[0].cmd === 'EXAMINE'
            || requests[0].cmd === 'SELECT'))
        state.status = STATES.BOXSELECTING;
...
ImapConnection.prototype.getAnnotation = function(name, entries, attributes, cb) {
  if (!this.serverSupports('ANNOTATEMORE'))
    return cb(new Error('Getting annotation is not supported on the server'));
  ...
  if ((!Array.isArray(entries) && typeof entries !== 'string')
    ...
    throw new Error('Entries argument must be a string or a non-empty Array');
  if ((!Array.isArray(attributes) && typeof attributes !== 'string')
      || (Array.isArray(attributes) && attributes.length === 0))
    throw new Error('Attrubutes argument must be a string or a non-empty Array');
  if (!Array.isArray(entries))
    entries = [entries];
  if (!Array.isArray(attributes))
    attributes = [attributes];
  var cmd = 'GETANNOTATION "';
  cmd += utils.escape(utf7.encode(''+name));
  cmd += '" ("';
  cmd += entries.map(utils.escape).join('" "');
  cmd += '") ("';
  cmd += attributes.map(utils.escape).join('" "');
  cmd += '")';
  this._send(cmd, cb);
};
...
ImapConnection.prototype._send = function(cmdstr, cb, bypass) {
  ...
  if ((cmdstr === undefined && this._state.requests.length)
    ...
    var reqs = this._state.requests;
    if(!bypass && (!this._state.pending || (reqs.length - this._state.pending)
        && !isSafeRequest(reqs[reqs.length - this._state.pending]))) // all requests are sent
      return;
    var prefix = '',
        cmd = (bypass ? cmdstr : reqs[reqs.length - this._state.pending].cmdstr);
    clearTimeout(this._state.tmrKeepalive);
    ...
    var cmds = [cmd];
    if(!bypass) --this._state.pending;
    // add all subsequent calls to the command list while previous call is concurrency-safe
    for(var i = reqs.length - this._state.pending; i < reqs.length
      && (!reqs[i-1] || isSafeRequest(reqs[i-1])); i++) {
      cmds.push(reqs[i].cmdstr);
      --this._state.pending;
    }
    ...
    cmds.forEach(function(cmd) {
      ...
      self._state.conn.write(prefix);
      self._state.conn.write(cmd);
      self._state.conn.write(CRLF);
      self.debug&&self.debug('\n==> ' + prefix + cmd + '\n');
    });
...
function isSafeRequest(request) {
  if(request) {
    var cmd = request.cmd == 'UID' ? request.cmdstr.split(' ', 2)[1] : request.cmd;
    return SAFE_CMDS[cmd];
  }
  return true;
}

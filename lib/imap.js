...
ImapConnection.prototype._send = function(cmdstr, cb) {
  ...
  if (cmdstr !== undefined) {
    ...
    if (cmdstr === 'IDLE' || cmdstr === 'DONE' || cmdstr === 'NOOP')
      reqs.splice(reqs.length - this._state.pending, 0, info);
    else
      reqs.push(info);
    ++this._state.pending;
  }
  ...
  if ((cmdstr === undefined && reqs.length) || reqs.length === 1
      || cmdstr === 'DONE') {
    var prefix = '', curReq = reqs[reqs.length - this._state.pending];
    if (cmdstr !== 'DONE' && (!this._state.pending || (reqs.length - this._state.pending)
    ...
    cmds.forEach(function(cmdstr) {
      if (cmdstr === 'IDLE') {
        ...
        prefix = 'IDLE ';
        self._state.ext.idle.state = IDLE_WAIT;
      } else if (cmdstr === 'NOOP')
        prefix = 'NOOP ';
      else if (cmdstr !== 'DONE')
        prefix = 'A' + (++self._state.curId) + ' ';

      self._state.conn.write(prefix + cmdstr + CRLF);
      self.debug&&self.debug('\n==> ' + prefix + cmdstr + '\n');
    });

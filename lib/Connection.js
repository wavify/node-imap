...
Connection.prototype.getAnnotation = function(name, entries, attributes, cb) {
  if (!this.serverSupports('ANNOTATEMORE'))
    throw new Error('Server does not support getting annotation');
  ...
  cmd += escape(utf7.encode(''+name));
  cmd += '" ("';
  cmd += entries.map(escape).join('" "');
  cmd += '") ("';
  cmd += attributes.map(escape).join('" "');
  cmd += '")';
  this._enqueue(cmd, function(err, result) {
    if (err)
      return cb(err);
    ...
    if (result) {
      for (var i = 0, len = result.length; i < len; ++i) {
        if (!ret[result[i].mailbox]) {
          ret[result[i].mailbox] = {};
        }
        Object.keys(result[i].entries).forEach(function(entryName) {
          ret[result[i].mailbox][entryName] = result[i].entries[entryName];
        });
      }
    }
...
Connection.prototype._lastRequest = function() {
  return this._curReq || this._sentQueue.length && this._sentQueue[this._sentQueue.length - 1];
}

Connection.prototype._isBlocking = function() {
  var req = this._lastRequest();
  if(req) {
    var cmd = req.type == 'UID' ? req.fullcmd.split(' ', 2)[1] : req.type;
...
Connection.prototype._processQueue = function() {
  // console.log('_processQueue', this._curReq, this._sentQueue.length, this._queue.length);
  if (!this._curReq && this._sentQueue.length) {
    this._curReq = this._sentQueue.shift();
  }
  if (!this._queue.length || !this._sock || !this._sock.writable || this._isBlocking())
    return;

  // skip IDLE/NOOP if there is another request in queue
  var q = this._queue, skip = 0;
  for (var last = q.length - 1;
    skip < last && (q[skip].type === 'IDLE' || q[skip].type === 'NOOP');
    ++skip)
    ;
  var doneIdle = !!this._sentQueue[0] && this._sentQueue[0].type === 'IDLE';
  if (doneIdle) this._sock.write('DONE' + CRLF);
  var req = q[skip];
  this._queue = q.slice(skip + 1);
  this._sentQueue.push(req);
